...
#define MAX_AUTHORITY	1.0f


#define SASPLAUSDIAG_START_SEC_VAR_CLEARED_BOOLEAN
#include "MemMap.h" /* PRQA S 5087 */
STATIC VAR(boolean, AP_SASPLAUSDIAG_VAR) SASPlausDiag_TestExecuted_Cnt_M_lgc; /* PRQA S 3218 */
#define SASPLAUSDIAG_STOP_SEC_VAR_CLEARED_BOOLEAN
#include "MemMap.h" /* PRQA S 5087 */
 ...
 * Symbol: SASPlausDiag_Per1
 *********************************************************************************************************************/

	VAR(boolean, AUTOMATIC) I2CHwPosValid_Cnt_T_lgc;
	VAR(float32, AUTOMATIC) I2CHwPos_HwDeg_T_f32;
	...
	if ((SASPlausDiag_TestExecuted_Cnt_M_lgc == FALSE) &&
		(I2CHwPosValid_Cnt_T_lgc == TRUE) && (VdAuthority_Uls_T_f32 >= MAX_AUTHORITY))
	{
		SASPlausDiag_TestExecuted_Cnt_M_lgc = TRUE;

		if (Abs_f32_m(I2CHwPos_HwDeg_T_f32 - VdHwPos_HwDeg_T_f32) > k_SASPlausDiagMaxDelta_HwDeg_f32)
		{
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_VLF_01, 1U, NTC_STATUS_FAILED);
		}
		else
		{
			(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_VLF_01, 0U, NTC_STATUS_PASSED);
		}
	}
