 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *          File:  Ap_BVDiag.c
 *     Workspace:  C:/SynergyWorkArea/CBD_BVDiag/BVDiag/autosar
 *     SW-C Type:  Ap_BVDiag
 *  Generated at:  Wed Oct 24 15:24:51 2012
 *
 ...
 * %version:           1 %
 * %derived_by:       rz3h1n %
 * %date_modified:    Thu Oct 25 11:41:22 2012 %
 *---------------------------------------------------------------------------------------------------------------------
															...
															VAR(uint16, AUTOMATIC) Option_T_u16)
{
	...
	{
		Rte_Call_SystemTime_GetSystemTime_mS_u32(SetTimer_T_ptr);
		Rte_Call_SystemTime_GetSystemTime_mS_u32(ClrTimer_T_ptr);
	}
	else
	{
		...
		if(Zone_T_u16 == CompareType_T_u16) 
		{
			...
			Rte_Call_SystemTime_GetSystemTime_mS_u32(SetTimer_T_ptr);
			Rte_Call_SystemTime_DtrmnElapsedTime_mS_u16(*ClrTimer_T_ptr, &ElapsedTime_T_u16p0);
			...
			if (ElapsedTime_T_u16p0 > ClrTimer_ms_T_u16p0)
			{
				...
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OpVoltageOvrMax, 0x0U, NTC_STATUS_PASSED);
				}
				else
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OpVoltage, 0x0U, NTC_STATUS_PASSED);
				}	
			}
		}
		else
		{
			...
			Rte_Call_SystemTime_GetSystemTime_mS_u32(ClrTimer_T_ptr);
			Rte_Call_SystemTime_DtrmnElapsedTime_mS_u16(*SetTimer_T_ptr, &ElapsedTime_T_u16p0);
			...
			if (ElapsedTime_T_u16p0 > SetTimer_ms_T_u16p0)
			{
				if (Option_T_u16 == D_DIAGOV_CNT_U16)
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OpVoltageOvrMax, 0x01U, NTC_STATUS_FAILED);
				}
				else
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OpVoltage, 0x01U, NTC_STATUS_FAILED);
				}	
			}
		}
	}
