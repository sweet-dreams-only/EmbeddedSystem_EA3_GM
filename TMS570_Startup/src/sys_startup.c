 ...
 * 06/23/16	 19		  AJM		PLL patch 																	EA3#9142
...
/***************************** INITIALIZATION STEP 06 *****************************/
	...
	else if(ResetCause_Cnt_Enum == PLLSLIP)
	{
		/* clear all reset status flags */
		systemREG1->SYSESR = 0xFFFF;
		/* continue with normal start-up sequence */
...
/***************************** INITIALIZATION STEP 07 *****************************/
	...
	uint32 NumOfRetries_Cnt_T_u32 = 0U;
	if (0 != ESMREGS->ESMSR3)
	{
...
/***************************** INITIALIZATION STEP 08 *****************************/
	if(0U != D_CHKFORPLLSLIP_CNT_U32(D_PLLSLIPWAITCYC_CNT_U32,&NumOfRetries_Cnt_T_u32))
	{
		/* PLL slip - Do a software reset*/
		ResetCause_Cnt_Enum = PLLSLIP;
		DiagFailedReset();
	}
	if((NumOfRetries_Cnt_T_u32 > 0U) && (ResetCause_Cnt_Enum!= PLLSLIP))
	{
		ResetCause_Cnt_Enum = PLLSLIPINFOONLY;
	}
